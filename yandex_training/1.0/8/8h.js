...
let getResult = (data) => {
	...
	let traverse = (currentNode) => {
		let left_height = currentNode.left ? currentNode.left.height + 1 : 0;
		let right_height = currentNode.right ? currentNode.right.height + 1 : 0;
		let balance = currentNode.height - Math.min(left_height, right_height);
		if (balance > 1) {
			isAVL = false;
		}
		if (currentNode.left !== null) {
