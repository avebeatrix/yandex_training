...
let getResult = data => {
	...
	let check = (m, checkparams) => {
		...
		let second_index = lLeftSearch(
			0, L-1,
			check2,
			[control_seq[m], second_seq]
		);
		if (second_seq[second_index]>control_seq[m]){
			second_index
		}
	...
	for (let i = 0; i < N; i++) {
		for (let j = i + 1; j < N; j++) {			
			let left_finded = lLeftSearch(
				0, L-1,
				check,
				[L, seqs[i], seqs[j]]
			);
			let right_finded = lLeftSearch(
				0, L-1,
				check,
				[L, seqs[j], seqs[i]]
			);
